...
Returns `Ws::Vector{SparseMatrixCSC{OnsiteOp{I}, Int}}` (one reduced bond
...
"""
function mpo_terms(
        Ws::Vector{<:SparseMatrixCSC{OnsiteOp{I}}},
    ...
    return map(1:N) do i
        ...
        for (idx, localop) in storedpairs(Ws[i])
            ...
            for (letter, coeff) in pairs(localop)
                ...
                # entries always address distinct slots; the `+=` only matters if an `OnsiteOp`
...
Whether the compressed MPO reproduces `H` exactly: `mpo_terms(irrep_mpo(H, sites, alg)...) ≈ H`.

This is the primary correctness check for a construction — cheap, purely symbolic, independent of `N`,
and valid for fermionic sectors, where densifying is not a well-defined operation. It compares
canonical term *sets* exactly and coefficients approximately, and ignores the lattice, since
[`mpo_terms`](@ref) knows the bond structure but not the physical spaces.
...
"""
function islossless(H::TermList, args...)
