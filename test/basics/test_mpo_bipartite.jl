...
const vertices5 = 1:5
const sites5 = fill(2, 5)   # qubit (dim-2) per site

@testset "mpo_bond_optimizations — single 2-site term" begin
    J = 2.0
    H = J * X[1] * X[2]
    Ws = mpo_bond_optimizations(vertices5, H)

    @test length(Ws) == 5
    # Bond dimension: trivially 1 everywhere (single term)
    @test all(==(1), minimum.(size.(Ws)))

    H_mpo = mpo_to_dense(Ws, sites5)
    H_dense = instantiate(H, sites5)
    @test H_mpo ≈ H_dense
end
...
@testset "mpo_bond_optimizations — Heisenberg chain" begin
    H = sum(X[i] * X[i + 1] + Y[i] * Y[i + 1] + Z[i] * Z[i + 1] for i in 1:4)
    Ws = mpo_bond_optimizations(vertices5, H)
    @test length(Ws) == 5
    @test mpo_to_dense(Ws, sites5) ≈ instantiate(H, sites5)
end
...
@testset "mpo_bond_optimizations — mixed coefficients" begin
    ...
    @test length(Ws) == 3
    @test mpo_to_dense(Ws, sites5[1:3]) ≈ instantiate(H, sites5[1:3])
end

@testset "mpo_bond_optimizations — single-site terms" begin
    H = sum(Z[i] for i in 1:5)
    Ws = mpo_bond_optimizations(vertices5, H)
    @test length(Ws) == 5
    @test mpo_to_dense(Ws, sites5) ≈ instantiate(H, sites5)
end
