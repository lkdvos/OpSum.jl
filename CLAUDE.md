...
1. **Symbolic operator algebra** — `src/operators/`
   ...
   - **`couple` is order-free under an abelian symmetry.** Storage is site-ordered, so an operand acting to the left of an earlier one has its leg *inserted* by `_couple_terms`: the running bond charges from the insertion point rightwards are recomputed (forced, by unique fusion) and the coefficient picks up `Rsymbol(c_a, c_b, c_a ⊗ c_b)` per leg it braids past — for fermionic sectors, exactly the anticommutation sign the caller used to hand-write. Gated on `_canreorder(I)` = `UniqueFusion` **and** `SymmetricBraiding`; non-abelian reordering needs F-moves and still throws. `dot` accepts either order for *any* symmetry (two legs to the unit sector need no F-move) and inserts the same scalar R-symbol — which is not always `+1`: it is `-1` for odd fermionic charges and for half-integer SU(2) charges, a sign the old sort-then-couple `dot` silently dropped.

2b. **Operator builders + hermitian conjugation** — `src/operators/builders.jl`
   - `spin_ops(V, sectors)` → `(; Sp, Sm, Sz)` for a U(1)-graded spin-`s` site; `sectors` is in **descending `m`**, because a `Vect[U₁]` spin site is as often labelled by particle number as by `m` and inferring would be a silent guess. `spin_ops(V, up, dn)` is the two-level form. `fermion_ops([V][, vac, occ])` → `(; c, cd, n)`. Both were re-derived by hand in nine files.
   - `spin` and `matrixunit` are memoised per space/sectors (`src/utility/memo.jl`), so the old "hoist them out of the loop" advice is obsolete. Safe because both are pure and `OnsiteOp` has no in-place API.
   - `hc(H[, sites])`: the hermitian-conjugate partner. It goes *through* the forward map — materialise the term's `K`-site block, adjoint it, `project` it back — rather than symbolically, because a letter's adjoint carries the dual charge and re-expressing the caterpillar coupler over dual charges is B-symbol data. Correct by construction (`project` checks its own faithfulness) and memoised per `(letters, tree, spaces)`, so it costs one projection per distinct term *shape*. Requires total charge `unit(I)`.
...
6. **MPO construction** — `src/operators/irrepmpo.jl`
   ...
   - The two verification helpers are public and live here too: `islossless(H[, sites][, alg])` is `mpo_terms(irrep_mpo(…)...) ≈ H`, and `mpo_tensormap(Ts)` contracts a chain of site tensors into `instantiate`'s convention. Both were hand-rolled in `test/testutils.jl`, `examples/common.jl` and `docs/src/operators.md`.
...
- **Faithfulness is `≈`**: `islossless(H, sites)` — i.e. `mpo_terms(irrep_mpo(H, sites)...) ≈ H` — compares canonical term *sets* exactly and coefficients approximately; it ignores the lattice, since `mpo_terms` knows the bonds but not the spaces.
...
- **Build local operators wherever reads best.** `spin`, `matrixunit`, `spin_ops`, `fermion_ops` are memoised; so is `hc` per term shape. Model code should never mention a bare `IrrepOperator(c, n)` — `n` follows TensorKit's block order, so hard-coding it is a latent bug.
...
- Shared test helpers live in `test/testutils.jl` (`LO`, `onlyterm`, `physmatrix`, `densedim`); `islossless` and `mpo_tensormap` used to be there and are now OpSum's public API.
