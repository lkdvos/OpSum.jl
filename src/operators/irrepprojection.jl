...
"""
    project(O::AbstractTensorMap, V::ElementarySpace; atol = 0, rtol = 100eps) -> OnsiteOp

Expand a single-site operator in the ITO alphabet of `V`, as an [`OnsiteOp`](@ref) that is not yet
placed on the lattice. `O` must be either `V ← V` or `V ← V ⊗ Vect[I](c => 1)`. The result carries
one letter per surviving alphabet element.

Because an `OnsiteOp` supports ordinary arithmetic, composite on-site operators read the way you
...
sentinel). An operator that is zero (or entirely below tolerance) gives the empty `OnsiteOp`, which
...
"""
    matrixunit(V::ElementarySpace, out::I, in::I) -> OnsiteOp
